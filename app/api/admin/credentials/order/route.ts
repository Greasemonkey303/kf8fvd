...
export async function POST(req: Request) {
  ...
  try {
    ...
    list = body.order.map((item, index) => {
      if (!item || typeof item !== 'object' || Array.isArray(item)) throw new Error(`order[${index}] invalid`)
      const id = Number((item as Record<string, unknown>).id)
      const sort_order = Number((item as Record<string, unknown>).sort_order || 0)
      if (!Number.isInteger(id) || id < 1) return { id: NaN, sort_order }
      if (!Number.isFinite(sort_order)) return { id, sort_order: NaN }
