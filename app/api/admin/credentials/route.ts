...
export async function POST(req: Request) {
  ...
  const insertRes = await query('INSERT INTO credentials (section, slug, s3_prefix, title, tag, authority, image_path, description, metadata, is_published, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [section, slug, s3Prefix, title, tag || null, authority || null, image_path || null, safeDescription, metadata, is_published ? 1 : 0, sort_order || 0])
...
export async function PUT(req: Request) {
  ...
  if (metadata !== undefined) {
    ...
    await query('UPDATE credentials SET section = ?, slug = ?, s3_prefix = ?, title = ?, tag = ?, authority = ?, image_path = ?, description = ?, is_published = ?, sort_order = ? WHERE id = ?', [section, slug, s3PrefixFinal || null, title, tag || null, authority || null, image_path || null, safeDescription, is_published ? 1 : 0, sort_order || 0, id])
  }
...
export async function DELETE(req: Request) {
  ...
  const rows = await query<{ s3_prefix: string }[]>('SELECT s3_prefix FROM credentials WHERE id = ?', [id])
