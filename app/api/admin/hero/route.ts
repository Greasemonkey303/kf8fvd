...
export async function GET(req: Request) {
  try {
    ...
    if (id) {
      const rows = await query<any[]>('SELECT * FROM hero WHERE id = ?', [Number(id)])
      return NextResponse.json({ items: rows })
    }
    const rows = await query<any[]>('SELECT * FROM hero ORDER BY id ASC')
    return NextResponse.json({ items: rows })
  } catch (err: unknown) {
...
export async function POST(req: Request) {
  try {
    ...
    if (body.id) {
      ...
      const rows = await query<any[]>('SELECT * FROM hero WHERE id = ?', [body.id])
      return NextResponse.json({ item: rows[0] })
    }
    await query<any[]>('INSERT INTO hero (title, subtitle, content) VALUES (?, ?, ?)', [body.title || '', body.subtitle || '', body.content || ''])
    const [newRow] = await query<any[]>('SELECT * FROM hero ORDER BY id DESC LIMIT 1')
