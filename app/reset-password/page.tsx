...
export default function ResetPasswordPage(){
  ...
  useEffect(()=>{
    ...
    pwDebounce.current = window.setTimeout(async ()=>{
      ...
      // dynamic import zxcvbn to avoid large bundle
      try {
        // @ts-ignore
      ...
      try {
        ...
        if (res.ok && j?.data) {
          // parse returned range for our suffix
        ...
        <Card id="reset-card" title="Reset Password" subtitle="Choose a new password">
          <form onSubmit={handleSubmit} className={styles.form} suppressHydrationWarning aria-labelledby="reset-card-title">
            ...
            <label>
              ...
              <input type="password" value={password} onChange={(e)=> setPassword(e.target.value)} className={styles.input} placeholder="New password" suppressHydrationWarning aria-describedby={strengthScore!=null || pwnedCount!=null ? 'pw-help' : undefined} />
