...
export default function LicenseCard(){
  ...
  const fallback = `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='1200' height='800'><rect width='100%' height='100%' fill='#0f172a'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='#fff' font-family='Arial' font-size='32'>Image unavailable</text></svg>`)}`
  ...
  return (
    <div className={styles.stationCard}>
      ...
      <div className={styles.mediaRow}>
        <button className={styles.thumbWrap} onClick={() => setOpen(err ? fallback : src)} style={{ padding:0, border:'none', background:'transparent' }} aria-label="Open Station License preview">
          {err ? (
